...
const TestCompleto = () => {
  ...
  const handleVisualResponse = (wasInStudy: boolean) => {
    ...
    if (currentVisualIndex < testImages.length - 1) {
      ...
      setTestResults(prev => ({ 
        ...
        memoria_inmediata: finalScore * 0.9
      }));
  ...
  const submitWorkingMemory = () => {
    try {
      ...
      if (workingMemoryTrial < 14) { // 15 trials total
        ...
        if (correct && workingMemoryTrial % 3 === 2 && sequenceLength < 8) {
  ...
  const respondToAttention = (isTarget: boolean) => {
    const newResponses = [...attentionResponses];
    newResponses[currentAttentionIndex] = isTarget;
    setAttentionResponses(newResponses);
  };

  const calculateAttentionScore = () => {
    try {
      // Asegurar que tenemos respuestas para todos los índices
      const completeResponses = attentionTargets.map((target, index) => {
        const userResponse = attentionResponses[index];
        return userResponse !== undefined ? userResponse === target : false;
      });
      
      const correct = completeResponses.filter(Boolean).length;
      const total = attentionTargets.length;
      ...
      setTestResults(prev => ({ 
        ...
        precision_respuestas: Math.max(prev.precision_respuestas || 0, precision),
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3">
                <h4 className="font-semibold">Tests incluidos:</h4>
      ...
      {currentPhase === 'visual' && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {visualPhase === 'study' ? 
                `Memorice este símbolo - ${currentVisualIndex + 1} de ${visualSequence.length}` :
                `¿Vio este símbolo antes? - ${currentVisualIndex + 1} de ${testImages.length}`
              }
            </CardDescription>
          </CardHeader>
            ...
            {visualPhase === 'study' ? (
              ...
              <div className="text-center space-y-6">
                ...
                <div className="flex justify-center space-x-4">
                  <Button 
                    onClick={() => handleVisualResponse(true)}
                    className="bg-green-600 hover:bg-green-700 px-8"
                  >
                    Sí, lo vi antes
                  </Button>
                  <Button 
                    onClick={() => handleVisualResponse(false)}
                    variant="outline"
                    className="px-8"
                  >
                    No, es nuevo
            ...
            {!workingMemoryStarted ? (
              ...
              <div className="text-center space-y-6">
                ...
                <Button 
                  ...
                  disabled={workingMemoryInput.length !== sequenceLength}
          ...
          <CardContent className="space-y-6">
            {!attentionStarted ? (
              ...
              <div className="text-center space-y-6">
                ...
                <Progress 
                  value={((currentAttentionIndex + 1) / attentionTargets.length) * 100} 
                  className="w-full"
                />
